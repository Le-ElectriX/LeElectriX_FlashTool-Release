...
对 ZLG 及兼容卡，扫描会按设备类型尝试打开并读取通道数。固件上报的类型字符串不必与内部枚举名完全一致；例如上报 `USBCAN-II` 的双通道兼容卡，只要通道数与探测类型一致，仍会出现在设备列表中（通常显示为 `ZLG_*`）。若固件名能明确解析为另一种已知设备类型（例如 CAN FD 型号名），则不会被记成错误的经典类型。
...
#### 5. 设置 Enpower 标定 CAN ID

设置界面的“标定发送 ID”和“标定接收 ID”用于配置 Enpower 标定通信使用的 CAN ID。手工编辑完成后会自动保存完整的发送/接收 ID 对，并在后续进入“MCU 标定”或“MCU 默认参数”页面时生效。从旧版本升级且尚未单独保存 ID 时，应用会在启动阶段从已选择的 MCU 参数文件读取并迁移 ID；设置页中显式保存的 ID 优先于参数文件中的值。

#### 6. 调整 Enpower 标定写指令间隔

设置界面的“标定写指令间隔（ms）”用于控制连续标定写指令之间的最小时间间隔，支持 `0` 至 `10000` 毫秒，默认值为 `0`。当控制器需要降低连续写入速度时，可适当增大该值；设置会自动保存并在后续进入标定页面时生效。

#### 7. 重置应用配置

设置界面的“重置配置”会在二次确认后清空全部已保存设置，包括许可证、标定参数文件路径、CAN ID 和写指令间隔。该操作无法撤销；重置成功后应用程序会自动关闭，下次启动时需要重新完成必要配置。

---
### MCU 标定参数保存

“保存参数到电脑”会导出当前参数文件中的完整标定参数集合，并按参数序号升序排列，不受参数组筛选、搜索结果或 Excel 原始顺序影响。

保存前必须先读取全部参数。只有完整参数集合都已在当前通信会话中取得有效值时才会生成文件；如果仍有未读参数，程序会提示“请先读取全部参数”，且不会创建或覆盖目标文件。这样可以区分尚未读取时的默认值 `0` 和 MCU 实际返回的真实值 `0`。保存操作本身不会自动初始化通信或补读参数。
